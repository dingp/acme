...
3. Create a Cronjob:
    ...
    - In container0 -> General:
        ...
        * Add the following environment variables:
            - `EMAIL` -> `<your_email>`
            - `DOMAIN` -> `<your domain name>` (multiple domain names can be separated by `:`), e.g. `site-ig.myowndomain.net`; do not include the auto-created `*.svc.spin.nersc.org` hostname if you want it to keep showing a certificate warning.
...
2. prepare a YAML file to configure the ingress for the domain names specified in the `$DOMAIN` environment variable; keep `$DOMAIN` limited to the user-facing hostnames that should appear on the certificate;
